...
fun KSlides.presentationMain_EN() {
    presentation {
        ...
        markdownSlide {
            content {
                """
                    ...
                    ${Mark.GREEN.block("Autorefleksja: Mam wrażenie, że używam cline nieprofesjonalnie")}
                    ${Mark.BLUE.block("Ja się na niego złoszczę (w prompcie)", indent = 1)}
                    ${Mark.BLUE.block("Tłumaczę, co zrobił źle (a wiem, że nie uczy się)", indent = 1)}
                    ${Mark.RED.block("A może jednak nie? Może ten brak profesjonalizmu ma sens?", indent = 1)}
                    ${vspace()}

                    ${Mark.GREEN.block("AI jeszcze nie zostało powszechnie zaadoptowane", fragmentIndex = 0)}
                    ${Mark.RED.block("Już jest źródłem problemów społecznych", indent = 1, fragmentIndex = 0)}
                    ${vspace()}

                    ${Mark.GREEN.block("Czy cline ma duszę?", fragmentIndex = 1)}
                    ${Mark.RED.block("Wypada jakoś poruszyć zajawkę z tytułu", indent = 1, fragmentIndex = 1)}
                    
                    Notes:
                    Będą trzy części, w tej właśnie kolejności.
                    - Jestem fanem programowania ze wsparciem mechanicznego programisty
                      - moja wydajność jest wyższa 3-5 razy; nie przepalam tylu tokenów
                    - Część I: mało jest filmów czy kursów, które pokazują na najbardziej podstawowym poziomie
                      - jak się do AI zwracać; jak wpisać prompt
                      - jak rozmawiać z AI; jak wpisać wpisać serię promptów
                      - nawet jak są, to często wydają mi się niewiarygodne
                        - przepisane bezmyślnie tezy i koncepty wyprodukowane na "chłopski rozum"
                      - Mam wrażenie, że nie używam AI profesjonalnie
                """.trimIndent()
            }
        }
            ...
            markdownSlide {
                ...
                content {
                    """
                        ...
                        ${Mark.RED.block("Wiedza techniczna koryguje antropomorfizację,")}
                        ${Mark.RED.block("ale jej nie wyłącza", indent = 1)}
                    """.trimIndent()
                }
            }
            dslSlide {
            ...
            markdownSlide {
                content {
                    """
                        ...
                        ${Mark.BLUE.block("🫣 Jednym z atrybutów Homo Sapiens")}
                        ${Mark.BLUE.block("jest posługiwanie się **językiem ze złożoną gramatyką**.")}
                        ...
                        ${Mark.RED.block("ani język ${charRArrow} wrażenie inteligencji, ani odwrotnie", indent = 1, fragmentIndex = 0)}
            ...
            markdownSlide {
                ...
                content {
                    """
                        ...
                        ${Mark.BLUE.block("LLM: \"Wow, to gada, to programuje, nawet z sensem\"")}
                        
                        ${Mark.RED.block("Nazwaliśmy tę namiastkę: AI - Artificial Intelligence")}
                        ${Mark.RED.block("To tak, jakby nazywać Ubera taksówką")}

                        ${vspace()}
                        ${Mark.GREEN.block("**Dobra nazwa ${charLongRArrow} entuzjazm, wycena, finansowanie**")}
        ...
        verticalSlides {
            ...
            dslSlide {
                ...
                content {
                    ...
                    span("block color_green") {
                        +"Na strukturalne prompty był hype "
            ...
            dslSlide {
                classes = "has_tip tip_at_line_3 end_of_thread"
                content {
                    blockQuote {
                        p {
                            strong { +"Interakcja:" }; br {}
            ...
            markdownSlide {
                content {
                    """
                        ...
                        Notes:
                        - Dałoby się nawet jakieś badania przywołać
                    """.trimIndent()
                }
            }
            markdownSlide {
                ...
                content {
                    """
                        ...
                        ${Mark.GREEN.block("Pokaż kotku co masz w środku ${Mark.BLUE("- black box nie ma flagi `--verbose`", 1)}", 0)}
            ...
            markdownSlide {
                content {
                    """
                        ...
                        ${Mark.RED.block("Wspieranie się AI staje się nową, wymaganą umiejętnością")}
        ...
        verticalSlides {
            markdownSlide {
                classes = "has_tip tip_at_line_34"
                content {
                    """
                        > Douglas Hofstadter, Gödel, Escher, Bach - "trip-let"${charBR}
                        > \-${charBR}
                        > niewspółprzywiedlność - ${charCopyright}J.Bralczyk

                        ${illustration("geb.jpg", alt = "GEB", size = IllustrationSize.Large)}

                        # Grande finale

                        ## Trzy różne opowieści o tym samym
                        ## ${charHellip} niewspółprzywiedlne
                        
                        ${vspace()}

                        ${Mark.BLUE.block("Poznawczo: ${Mark.GREEN("Rozmawiamy z AI jak z \"kimś\", nie mamy wyrobionych instynktów")}")}
